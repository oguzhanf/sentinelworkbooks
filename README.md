...
**Author:** Omar Al Homaidi ([@ohomaidi](https://github.com/ohomaidi))
...
|---|----------|-------------|------------------|
| 01 | **Perimeter Breach Detection** | Correlates firewall allow/deny events with downstream web server activity to confirm perimeter breaches | CommonSecurityLog, W3CIISLog |
| 02 | **Identity-to-Network Kill Chain** | Reconstructs NAC/RADIUS auth → Windows logon → server access chains; detects credential relay | CommonSecurityLog (Cisco ISE), SecurityEvent, Syslog |
| 03 | **Data Exfiltration Across Layers** | Multi-layer data loss detection across proxy, IIS, Entra audit, and Unix file activity | CommonSecurityLog (Squid), W3CIISLog, AuditLogs, Syslog |
| 04 | **Lateral Movement & Reconnaissance** | Detects internal movement via Windows logons, Unix auth, and firewall internal traffic | SecurityEvent, Syslog, CommonSecurityLog |
| 05 | **Web Application Attack Chain** | Correlates WAF blocks/allows with web server errors to detect successful exploitation | CommonSecurityLog (WAF), W3CIISLog, Syslog |
| 06 | **Infrastructure Availability vs Security** | Distinguishes DDoS/attacks from outages by correlating NMS device-down events with firewall spikes | Syslog, CommonSecurityLog, Heartbeat |
| 07 | **Privileged Account Abuse** | Monitors admin activity across Windows, Unix, network, and cloud within 30-minute windows | SecurityEvent, Syslog, CommonSecurityLog, AuditLogs |
| 08 | **DNS as an Attack Vector** | InfoBlox-centric detection: DNS resolution → firewall connection correlation for C2 confirmation | CommonSecurityLog (InfoBlox), Syslog |
| 09 | **Cloud & Hybrid Threat Correlation** | Detects cloud-pivoted attacks: Azure resource deletion → on-prem logon → firewall allow | AzureActivity, SecurityEvent, CommonSecurityLog, AuditLogs |
| 10 | **SOC Health & Coverage** | Tracks data ingestion health, agent connectivity, connector status, and ingestion latency | Heartbeat, SentinelHealth, CommonSecurityLog, Syslog |
...
|---|----------|-------------|------------------|---------|
| 01 | **Unified Security Posture** | Executive view across MDE, Entra ID, Defender for Cloud | SecurityAlert, DeviceInfo | v2 |
| 02 | **Identity Attack Chain** | Traces Entra sign-ins → AD → endpoint logons with lateral movement detection | SigninLogs, AADNonInteractiveUserSignInLogs, AuditLogs | v1 |
| 03 | **Privileged Access & Admin Abuse** | Monitors PIM activations, Azure privileged ops, O365 admin activity, service principals | AuditLogs, AzureActivity, AADServicePrincipalSignInLogs | v1 |
| 04 | **Endpoint Compromise & Lateral Movement** | Full post-compromise chain: processes, connections, files, and lateral movement via MDE | AlertInfo, DeviceProcessEvents, DeviceNetworkEvents, DeviceFileEvents | v2 |
| 05 | **Endpoint Health & Coverage** | MDE deployment state, patch gaps, exposure levels, and sensor health | DeviceInfo, SecurityRecommendation, DeviceVulnerabilityAssessment | v3 |
| 06 | **Phishing to Compromise Chain** | Full chain from email delivery → URL clicks → ZAP actions → account/device compromise | EmailEvents, EmailUrlInfo, SigninLogs, SecurityAlert | v4 |
| 07 | **Data Exfiltration & Insider Risk** | Data theft via SharePoint/OneDrive, email forwarding, endpoint files, and cloud uploads | OfficeActivity, DeviceFileEvents, EmailEvents | v1 |
| 08 | **Cloud Threat & Azure Anomaly** | Azure control plane monitoring, service principal anomalies, privileged Entra operations | AzureActivity, AADServicePrincipalSignInLogs, AuditLogs | v2 |
| 09 | **SOC Incident Investigation Helper** | Parameter-driven unified entity investigation with user and device timelines | SecurityAlert, SigninLogs, AuditLogs, OfficeActivity, DeviceProcessEvents | v2 |
| 10 | **Threat Hunting Dashboard** | Proactive hunting: rare processes, LOLBin abuse, first-time-seen IPs, suspicious logons | DeviceProcessEvents, DeviceNetworkEvents, DeviceLogonEvents, DeviceFileEvents | v1 |
